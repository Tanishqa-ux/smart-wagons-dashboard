...
5. Set `VITE_TRAIN_RUNNING_AFTER_SECONDS`. This controls the train running/not-running board. `60` is a good first value.
6. Run:
...
The dashboard marks a train as `Running` when that device has a latest reading inside `VITE_TRAIN_RUNNING_AFTER_SECONDS`. It also shows the exact `Last Active` date and time from Supabase for every train/device.
